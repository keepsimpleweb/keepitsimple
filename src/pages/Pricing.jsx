...
import { Stack, Icon, HStack, Button, Flex, Text, VStack, Box, Heading } from '@chakra-ui/react';
...
const Card =({ children, price, plan, f,fi})=>{
	const sty = {
...
return ( <> <Animation>
  <VStack as='section' overflow={'hidden'}>
    <Box as='article' color='white' fontSize={sty.res} pt={'30px'} mx={6}>
      <Heading color='white' fontWeight='extrabold' fontSize={[20,20,30]} pb={2}>
        {plan} pricing for your business
      </Heading>
      <Text fontSize={'0.6em'} fontWeight='medium' pb={6}>
        Plans that are carefully crafted to suit your business.
      </Text>
      <Box as='article' maxW='870px' margin='auto' 
        borderRadius='xl' overflow='hidden' bg='white'
      ...
      >
        <Flex direction={sty.lay} justifyContent='center' pt={2}>
          <Box bg='#F0EAFB' color='black' py='60px' h='350px'
            w={{base: '420px', sm: '420px',md: '420px', lg: '420px'}} 
            direction='column' margin='auto' textAlign='center' mt={-4}
            justifyContent={'center'} overflow='hidden'
          >
            <Text fontSize='3xl' fontWeight='extrabold' >{plan}</Text>

            <Heading as='h3' fontSize='6xl' mt='4' >${price}</Heading>

            <Text fontSize='medium' mt='2'
              fontWeight='medium' color='#171923' 
            >billed just one</Text>

            <Link to='/register'>
              <Button textAlign={sty.Tali} fontSize='medium' size='lg' px={'82px'}
                fontWeight='medium' colorScheme='purple' mt='6' mb='8' borderRadius='3xl' boxShadow='md'
                sx={{backdropFilter: 'blur(10px)', boxShadow: '0px 0px 4px purple'}}
              >Get started</Button>
            </Link>
          </Box>

          <Box w='440px' margin='auto' color='black' borderRadius='xl' pb={8}> 
            <Text fontSize={'0.6em'} fontWeight='medium' px={'40px'} mt={4}>
              Access these features when you get this pricing package for your business.
            </Text>

            <Box color='black' textAlign={'left'} mt={5}> 
              <Text fontSize={'0.6em'} fontWeight='medium' px={8} mt={8} ml={4}>
                <CheckCircleIcon color='purple' mt={-1}/> International calling API
              </Text>
              <Text fontSize={'0.6em'} fontWeight='medium' px={8} mt={4} ml={4}>
                <CheckCircleIcon color='purple' mt={-1}/> Design for many screens
              </Text>
              <Text fontSize={'0.6em'} fontWeight='medium' px={8} mt={4} ml={4}>
                <CheckCircleIcon color='purple' mt={-1} sx={{filter: fi}}/> Automated messages via Slack
              </Text>
              <Text fontSize={'0.6em'} fontWeight='medium' px={8} mt={4} ml={4}>
                <CheckCircleIcon color='purple' mt={-1} sx={{filter: f}}/> 24/7 Support and consulting
              </Text>
            </Box>
          </Box>
        </Flex>
...
export function Features() { return (
  <Box maxW='1024px' m='auto' pt='30px' pb='8' as='section'>
    <Stack px='12' spacing='5' color='white'  direction={['column','column','row']} pl={[0,10,24,20]}>
      <Feature icon={MoneyBackGuaranteeIcon}>
        30 days money back Guarantee
      </Feature>
      <Feature icon={HassleFreeIcon}>No setup fees 100% hassle-free</Feature>
      <Feature icon={MonthlySubscriptionIcon}>
        Scalability and performance for monthly subscription for unlimited users
      </Feature>
    </Stack>
  </Box>
); };

export function CheckIcon() { return (
  <svg
    width='22'
    height='22'
    viewBox='0 0 22 22'
...
function MonthlySubscriptionIcon({ ...props }) {
return (
  <svg
    {...props}
    width='40'
